...
namespace WebApp.Configurations.Services
{
    public class ExecutionOrchestrator : IExecutionOrchestrator
    {
        ...
        public async Task<ResponsePaged<TResponse>> TryCatchTransactionalAsync<TRequest, TResponse>
          (Func<Task<ResponsePaged<TResponse>>> func, string method, TRequest request, Func<Exception, ResponsePaged<TResponse>, ResponsePaged<TResponse>> handleError = null, string errorMessage = null)
          where TRequest : ITracking
        {
            var response = Activator.CreateInstance<ResponsePaged<TResponse>>();
            try
            {
                response.TrackingId = request?.TrackingId ?? Guid.Empty;
                var error = false;
