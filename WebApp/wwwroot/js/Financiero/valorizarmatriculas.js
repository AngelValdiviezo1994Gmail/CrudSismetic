...
function GuardarSeleccionResolucionesCostos() {
    ...
    for (var i = 0; i < allGridItems.length; i++) {
        ...
        SumaCantidades = SumaCantidades + valor;
        if (CodMat == "" || Concepto == "") {
    ...
    */
    ...
    if (vorden != null) {
        $.ajax({
            ...
            //url: "/OrdenMatricula/ValorizarOrden",
...
function CargarDatosOrden() {
    ...
    let orden = {};
    orden.OrdenMatValor = $("#OrdenMatValor").val();
    ...
    //orden.OrdenMatFechaLimitePagoOrdinario = moment($('#OrdenMatFechaLimitePagoOrdinario').val(), "DD/MM/YYYY");
    //orden.OrdenMatFechaLimitePagoExtra = moment($('#OrdenMatFechaLimitePagoExtra').val(), "DD/MM/YYYY");
    //orden.EstId = $("#MatId").val();
    /*
    ...
    */
    ...
    let lstTmpMatDet = [];
    let elementDet = document.getElementById("gridDetalleOrdenMat");
    let instanceDet = DevExpress.ui.dxDataGrid.getInstance(elementDet);
    var allGridItemsDet = instanceDet.getDataSource().items();

    for (var i = 0; i < allGridItemsDet.length; i++) {
        var row = allGridItemsDet[i];
        lstTmpMatDet.push(row.concepCostoId);
    }

    orden.DetMatricula = lstTmpMatDet;
    ...
    if (orden.Matriculas.length == 0 || orden.DetMatricula.length == 0 || _OrdenMatValor == null || _OrdenMatValor == ""
        ...
        || orden.OrdenMatFechaLimitePagoExtra == null || orden.OrdenMatFechaLimitePagoExtra == "") {
        retornaDatos = false;
    }
...
/*
function ComparacionFechas(date1, date2) {
    var x = new Date();
    var fecha = date.split("/");
    x.setFullYear(fecha[2], fecha[1] - 1, fecha[0]);
    var today = new Date();
...
function MostrarConcesiones() {
    ...
    let element = document.getElementById("gridOrdenMatValorizar");
    let instance = DevExpress.ui.dxDataGrid.getInstance(element);
    var allGridItems = instance.getSelectedRowsData();
...
function AplicarConcesiones() {
    ...
    if (IdMatSession != null && IdMatSession != "") {
        ...
        let elementEstudiante = document.getElementById("gridOrdenMatValorizar");
...
function GuardarSeleccionResolucionesCostos() {
    ...
    for (var i = 0; i < allGridItems.length; i++) {
        ...
        if (valor > 0) {
            var arreglo = row.concepCostoId + "," + row.concCostoCantidad + "," + row.concepCostoDescripcion + "," + row.concepCostoValor + "," + row.concepCostoCodigo + "," + row.provNombre + ";";
            LstConceptos.push(arreglo);
        }
    }
...
function AnulacionSeleccionOrdenMatricula() {
    ...
    let element = document.getElementById("gridOrdenMatValorizar");
    //let element = document.getElementById("gridOrdenMatCreadas");
    let instance = DevExpress.ui.dxDataGrid.getInstance(element);
    var allGridItems = instance.getSelectedRowsData();
...
function Cancelar() {
    MensajeGeneral("", "", "Se cancela la creación de orden de matrícula", respuesta.botonAceptar, GetURL() + "OrdenMatricula/Index");
}
...
function RechazoPago() {
    ...
    $.ajax({
        ...
        */
        ...
        error: function (xhr, ajaxOptions, thrownError) {
            MensajeGeneral("error", "ERROR ENVÍO DE ÓRDENES", "Se presento un error en el consumo del método de rechazo de pago de órdenes", "Aceptar");
        }
    });
